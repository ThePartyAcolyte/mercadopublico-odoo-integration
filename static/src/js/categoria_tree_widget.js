...
export class CategoryCheckbox extends Component {
    static template = "MercadoPublico.CategoryCheckbox";
    static props = ["id", "checked", "indeterminate", "onChange"];

    setup() {
        this.inputRef = useRef("input");
        useEffect((checked, indeterminate) => {
            if (this.inputRef.el) {
                this.inputRef.el.checked = checked;
                this.inputRef.el.indeterminate = indeterminate;
            }
        }, () => [this.props.checked, this.props.indeterminate]);
    }
}

export class CategoriaTreeWidget extends Component {
    static template = "MercadoPublico.CategoriaTreeWidget";
    ...
    get selectedIds() {
        // En Odoo 18, un x2many en la prop record.data[name] es un RecordList
    ...
    async toggleSelection(node, ev) {
        ...
        // Update the record with replace command
...
export const categoriaTreeWidget = {
    component: CategoriaTreeWidget,
    supportedTypes: ["many2many"],
};

registry.category("fields").add("categoria_tree", categoriaTreeWidget);
